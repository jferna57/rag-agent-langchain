...
def obtener_info_gpu():
    ...
    try:
        ...
        return {"numero_gpus": 0, "nombres": [f"Error al obtener info de GPU: {e}"]}

def guardar_info_en_json(info, nombre_archivo="info_equipo.json"):
    """Guarda la información del equipo en un archivo JSON."""

    with open(nombre_archivo, "w") as archivo_json:
        json.dump(info, archivo_json, indent=4)  # Indent para formatear el JSON

# Ejemplo de uso
if __name__ == "__main__":
    info_equipo = obtener_info_equipo()
    guardar_info_en_json(info_equipo)
